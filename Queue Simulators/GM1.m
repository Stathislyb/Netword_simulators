function [mean_system_clients, mean_server_clients, mean_queue_clients, mean_delay_system, mean_delay_queue, mean_delay_server] = GM1( a, m, Sim_Time )
...
while flag
    ...
    if event==1                                 % Birth event
        ...
        if server_busy==0
            ...
            server_busy=1;
			%  call the event 2 after random exponential time with mean 1/m
		...
		% call event 1 for new birth after random time with mean a
		Event_List(1,end+1)=1;
		Event_List(2,end)=T+2*a*rand;		% rand with range from 0 to 2a and having mean a
    ...
    elseif event==2                             % Death event
        ...
        if pending_clients>0
			...
			%  after random exponential time with mean 1/m
            Event_List(1,end+1)=2;
			temp_death_time = exprnd(1/m,1,1);
