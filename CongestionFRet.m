function [ ] = CongestionFRet( Sim_Time, Threshold, Min_Ack_Time, Max_Ack_Time, Time_Out, Drop_Probability)
format longG;
Sim_Flag=true;
Time=0;
successful_ack=0;
...
while Sim_Flag
    ...
    if event==1
        ...
        if rand() < Drop_Probability
            % Ean prolavei na anagnorisei tin aporipsi tou segment mesa se 3 ack. tote tha sunexisei kanonika
			% allios tha kanei time out
			temp_time = Time + 3*(rand()*(Max_Ack_Time - Min_Ack_Time) + Min_Ack_Time);
			if temp_time >= Time + Time_Out
    ...
    elseif event==3
        ...
        Congestion_Window = 1;
		%disp('Congestion Window has been reseted');    % minima gia epanafora tou parathirou stin arxiki tou timi
